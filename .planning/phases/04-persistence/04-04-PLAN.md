...
must_haves:
  truths:
    ...
    - "Circuit reconnect triggers state re-restoration in Server mode"
  artifacts:
    ...
    - path: "tests/Bustand.Tests/Persistence/CircuitReconnectTests.cs"
      provides: "Tests for circuit reconnect state restoration (PERS-04)"
      contains: "class CircuitReconnectTests"
  key_links:
    ...
    - from: "tests/Bustand.Tests/Persistence/CircuitReconnectTests.cs"
      to: "BustandCircuitHandler"
      via: "circuit lifecycle testing"
      pattern: "CircuitHandler"
---

<objective>
Comprehensive test suite for persistence functionality

Purpose: Ensures all persistence components work correctly in isolation and together. Covers debouncing behavior, storage service error handling, middleware integration, end-to-end persistence flow, and circuit reconnect state restoration for Blazor Server.

Output: Unit tests for DebouncedWriter, BrowserStorageService, PersistenceMiddleware, plus integration tests for the full persistence workflow including DI registration and circuit reconnect scenarios.
</objective>
...
BrowserStorageServiceTests covers OnAvailabilityChanged event and SetUnavailable.
...
<task type="auto">
  <name>Task 3: Create integration tests and circuit reconnect tests</name>
  <files>
    tests/Bustand.Tests/Persistence/PersistenceIntegrationTests.cs
    tests/Bustand.Tests/Persistence/CircuitReconnectTests.cs
  </files>
...
Create CircuitReconnectTests.cs for PERS-04 coverage:
```csharp
...
public class CircuitReconnectTests
{
    ...
    [Fact]
    public async Task OnConnectionUp_CallsSetAvailable()
    {
        // Arrange
        var jsRuntime = Substitute.For<IJSRuntime>();
    ...
    [Fact]
    public async Task OnConnectionDown_MarksStorageUnavailable()
    {
        // Arrange
        var jsRuntime = Substitute.For<IJSRuntime>();
        var storage = new BrowserStorageService(jsRuntime);
    ...
    [Fact]
    public async Task OnCircuitClosed_MarksStorageUnavailable()
    {
        // Arrange
        var jsRuntime = Substitute.For<IJSRuntime>();
        var storage = new BrowserStorageService(jsRuntime);
    ...
    [Fact]
    public async Task CircuitReconnect_RaisesOnAvailabilityChanged()
    {
        // Arrange
        var jsRuntime = Substitute.For<IJSRuntime>();
        var storage = new BrowserStorageService(jsRuntime);
    ...
    [Fact]
    public async Task CircuitLifecycle_FullCycle_HandlesCorrectly()
    {
        // Arrange
        var jsRuntime = Substitute.For<IJSRuntime>();
        var storage = new BrowserStorageService(jsRuntime);
        ...
        // 2. First render happens (simulated by SetAvailable)
        storage.SetAvailable();
        Assert.True(storage.IsAvailable);
        ...
        // Assert - availability changed events (not counting initial SetAvailable since we subscribe after)
        // Events: SetAvailable(true), OnConnectionUp(true) = 2 true events
    ...
    /// Note: Circuit is a sealed class, so we use a workaround.
    /// </summary>
    private static Circuit CreateMockCircuit()
    {
        // Circuit is sealed with internal constructor, so we need to create it via reflection
        // or use a test double approach. For simplicity, we'll create a minimal mock.
        // In real tests, you might need to use a mock framework that supports sealed classes.

        // Using reflection to create Circuit instance (it has internal constructor)
        var circuitType = typeof(Circuit);
        var constructor = circuitType.GetConstructors(
        ...
        if (constructor != null)
        {
            return (Circuit)constructor.Invoke(new object[] { "test-circuit-id" });
        }

        // Fallback: If we can't create a real Circuit, the tests will need adjustment
        // For now, return a mock that satisfies the interface
        return Substitute.For<Circuit>();
    }
}
```
  </action>
  <verify>
dotnet build tests/Bustand.Tests/Bustand.Tests.csproj --no-restore
dotnet test tests/Bustand.Tests/Bustand.Tests.csproj --filter "FullyQualifiedName~PersistenceIntegrationTests|FullyQualifiedName~CircuitReconnectTests" --no-build
...
CircuitReconnectTests covers PERS-04 (circuit reconnect state restoration).
Tests verify OnConnectionUp triggers SetAvailable and OnAvailabilityChanged.
Tests verify circuit lifecycle events properly track storage availability.
All tests pass.
  </done>
</task>
...
3. Test coverage includes unit tests, integration tests, and circuit reconnect tests
4. Graceful degradation scenarios tested
5. PERS-04 (circuit reconnect) properly tested
</verification>

<success_criteria>
- DebouncedWriterTests covers batching, timing, flush, dispose
- BrowserStorageServiceTests covers availability, JS interop, error handling, OnAvailabilityChanged, SetUnavailable
