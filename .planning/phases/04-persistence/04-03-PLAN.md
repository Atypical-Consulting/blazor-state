...
Purpose: Integrates persistence into the Bustand DI registration flow. Stores marked with [Persist] automatically get PersistenceMiddleware configured and state restored from storage on first access.

Output: Updated ServiceCollectionExtensions to register IBrowserStorage and configure PersistenceMiddleware for [Persist] stores. Updated ZustandStore with hooks for state restoration.
</objective>
...
<task type="auto">
  <name>Task 2: Update ServiceCollectionExtensions for persistence registration</name>
  <files>
    src/Bustand/Extensions/ServiceCollectionExtensions.cs
  </files>
  <action>
