...
files_modified:
  ...
  - src/Bustand/Configuration/StoreLifetime.cs
...
<task type="auto">
  <name>Task 2: Create mode detector and DI extensions</name>
  <files>
    src/Bustand/Detection/BlazorModeDetector.cs
    src/Bustand/Extensions/ServiceCollectionExtensions.cs
  </files>
  <action>
Create the mode detection and DI registration logic:
...
2. Create `src/Bustand/Extensions/ServiceCollectionExtensions.cs`:
```csharp
using System.Reflection;
...
3. Ensure proper namespaces are exported. Update or create `src/Bustand/GlobalUsings.cs` (if needed for cleaner imports in consuming projects):
```csharp
// This file intentionally left minimal
// Consumers import specific namespaces as needed
```

Note: The Scrutor `WithAttribute<T>()` method filters to only classes with the attribute. The `AssignableTo(typeof(IStore))` adds an extra safety check.
  </action>
  <verify>
`dotnet build src/Bustand/Bustand.csproj` succeeds, verify ServiceCollectionExtensions contains AddBustand method
  </verify>
  <done>
BlazorModeDetector.IsWebAssembly works, AddBustand() extension exists with Scrutor scanning, per-store lifetime overrides are applied
  </done>
</task>

<task type="auto">
  <name>Task 3: Add public API exports and verify integration</name>
  <files>
    src/Bustand/Bustand.cs
  </files>
  <action>
Create a root namespace file that re-exports commonly used types for convenient importing:

1. Create `src/Bustand/Bustand.cs`:
```csharp
...
2. Verify the complete integration by reviewing the build output.
...
```
  ...
  <verify>
`dotnet build src/Bustand/Bustand.csproj` succeeds with no warnings, all public types are accessible
  </verify>
  <done>
Project builds cleanly, public API surface is minimal and well-organized: ZustandStore<T>, BustandStoreAttribute, AddBustand() extension
  </done>
</task>
...
6. Per-store lifetime overrides are applied correctly
7. Warning is logged for Singleton in Server mode
</success_criteria>
